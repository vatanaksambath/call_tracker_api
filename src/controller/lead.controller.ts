...
export class LeadController {
    ...
    @UsePipes(new ValidationPipe())
    @HttpCode(200)
    async create(@Body() LeadDTO: LeadDTO, @Req() req) {
        const userId = req.user?.user_id;
       try {
    ...
    @UseGuards(PermissionGuard)
    @RequirePermission('MU_04', 'PM_03')
    @UseGuards(AuthGuard)
    @Put('update')
    @UsePipes(new ValidationPipe())
    @HttpCode(200)
    async update(@Body() LeadDTO: LeadDTO, @Req() req) {
        const userId = req.user?.user_id;
        try {
    ...
    async export(@Body() commonDto: CommonDTO, @Req() req) {
        const userId = req.user?.user_id;
       try {
            const result = this.leadService.LeadExport(commonDto, userId);
            return result;
        } catch (error) {
            dispatchBadRequestException(error);
        }
